...
STATE_REGISTRY: dict[FgoState, FgoStateDef] = {
    ...
    FgoState.SKILL_CONFIRM: FgoStateDef(
        FgoState.SKILL_CONFIRM,
        ["battle/skill_confirm"],
        minimum_confidence=0.85,
        timeout_seconds=5.0,
        on_timeout=ON_TIMEOUT_ERROR,
    ),
    FgoState.ALLY_TARGET_SELECT: FgoStateDef(
        FgoState.ALLY_TARGET_SELECT,
        ["battle/ally_target_select"],
        minimum_confidence=0.85,
        timeout_seconds=5.0,
        on_timeout=ON_TIMEOUT_ERROR,
    ),
    FgoState.ENEMY_TARGET_SELECT: FgoStateDef(
        FgoState.ENEMY_TARGET_SELECT,
        ["battle/enemy_target_select"],
        minimum_confidence=0.85,
        timeout_seconds=5.0,
        on_timeout=ON_TIMEOUT_ERROR,
    ),
    FgoState.MASTER_SKILL_PANEL: FgoStateDef(
        FgoState.MASTER_SKILL_PANEL,
        ["battle/master_skill_panel"],
        minimum_confidence=0.85,
        timeout_seconds=5.0,
        on_timeout=ON_TIMEOUT_ERROR,
    ),
    FgoState.ORDER_CHANGE_SELECT: FgoStateDef(
        FgoState.ORDER_CHANGE_SELECT,
        ["battle/order_change_select"],
        minimum_confidence=0.85,
        timeout_seconds=5.0,
        on_timeout=ON_TIMEOUT_ERROR,
    ),
